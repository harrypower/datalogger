...
: dth11_start_signal piosetup throw 25 pipinsetpulldisable throw 25 pipinhigh throw 25 pipinoutput throw 25 pipinhigh throw 25 pipinlow throw 9000 wait 25 pipinhigh throw 25  pipininput throw  ;
...
: get_temp_humd ( -- ntemp nhumd nerror )
    dth11_busy? false =
    if
	retry_times { ntimes } 1 2
	begin
	    drop drop
	    ntimes 0 >
	    if
		ntimes 1 - to ntimes
		dth11_parse dup 0 = if true else false 2000 ms then
...
: config-dth-type
    ...
    s" -22" search
    if
	22 to dth_11_22_? 2drop get_temp_humd bye
    then
    s" -11" search
    if
	11 to dth_11_22_? 2drop get_temp_humd bye
    then ;

: seeit dth11_size dup 50 - ?do i dup . dth11_data_retrieve . cr loop ;
